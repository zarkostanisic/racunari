...
<title>Prodavnica kompjutera</title>
<?php
   ...
   <div class="center_content">
		<?php
			...
			if(isset($_POST['unesi_komentar'])){
				...
				$ispis_izmena = "";
				if(count($greske)==0){
					$upit_izmena_komentara="UPDATE komentari SET komentar='$komentar' WHERE id_komentara='$id' OR id_komentara='$id_komentara'";
					$rezultat_izmena_komentara=mysql_query($upit_izmena_komentara,$konekcija);

					if($rezultat_izmena_komentara){
						$ispis_izmena.="<div class='komentar' style='text-align:center;'>Komentar je uspešno izmenjen</div>";
					}else{
