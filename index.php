...
<div id="main_container">
   ...
   <div class="center_content">
		<?php
			$upit_proizvodi="SELECT p.id_proizvoda, p.id_podkategorije, p.naziv_proizvoda, p.cena_proizvoda, p.slika_proizvoda, p.nova_cena FROM proizvodi p JOIN podkategorije po ON p.id_podkategorije=po.id_podkategorije WHERE p.akcija='da' AND p.status='1' ORDER BY RAND() LIMIT 9";
			$rezultat_proizvodi=mysql_query($upit_proizvodi,$konekcija);
			$broj_proizvodi=mysql_num_rows($rezultat_proizvodi);

			echo "<div class='center_title_bar'>Izdvajamo iz ponude</div>";
			if($rezultat_proizvodi){
				if($broj_proizvodi>0){
					...
					echo "<div class='komentar' style='margin-left:70px;text-align:center;'>Trenutno nema izdvojenih proizvoda</div>";
				}
			}
		?>
